...
class IntProcessor:
    ...
    def store_input(self):
        ...
        self.set_value(pos, self.input_values[0])
...
p = IntProcessor(example1, SINGLE_LOOP_MODE)
p.run([]) | eq([109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99])

p = IntProcessor(example2, SINGLE_LOOP_MODE)
[output] = p.run([])
len(str(output)) | eq(16)

p = IntProcessor(example3, SINGLE_LOOP_MODE)
p.run([]) | eq([1125899906842624])
